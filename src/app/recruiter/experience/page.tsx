...
export default async function RecruiterExperiencePage() {
  const experiences = await getExperiences();

  return (
    <SubPageLayout title="Erfaring" backHref="/recruiter" backLabel="Rekrutterer">
          ...
          <div key={exp._id} className="relative flex gap-6">
            ...
            >
              <div className="flex items-start justify-between gap-4 flex-wrap mb-2">
                <div>
                  <p className="text-lg font-bold" style={{ color: "var(--foreground)", fontFamily: "var(--font-heading)" }}>
                    {exp.role}
                  </p>
                  <p className="text-sm font-medium mt-0.5" style={{ color: "var(--accent)", fontFamily: "var(--font-body)" }}>
                    {exp.company}
                  </p>
                </div>
